...
class Trainer():
    ...
    def calc_gp(self, fake_inputs, real_inputs):
        ...
        gradients = grad(outputs=y_pred_interpolates, inputs=interpolates, 
    ...
    def critic_train_iteration(self):
        ...
        real_inputs = one_hot(torch.tensor(next(self.walker.walk())), 
                              num_classes=self.N).type(torch.float64).to(self.device)
        
        y_pred_fake = self.discriminator(fake_inputs)
        y_pred_real = self.discriminator(real_inputs)
        gp = self.calc_gp(fake_inputs, real_inputs)
